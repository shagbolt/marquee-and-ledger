...
export function confirmGreenlight(){
    ...
    var movie = {
      ...
      // Genre specialty bonuses are folded in here too, for the same reason: a match
      // helps this picture, not the person's fee or future films.
      effWriterSkill: applySpecialtyBonus(sel.w.skill, sel.w, genre),
...
export function openProductionEvent(movie){
    ...
    btns.forEach(function(btn){
      btn.addEventListener('click', function(){
        ...
        movie.eventLog.push({ phase:'Pre-Production', week:game.processedWeek, title:evt.title, choice:choice.label, outcome:outcome });
        renderHeader();
...
export function showEventOutcome(outcome, movie){
    renderDecisionOutcome(outcome, 'Continue to Production →');
...
export function openShootEvent(movie, shoot){
    ...
    renderDecisionCard('🎥 ON-SET DECISION', evt, movie);
    var btns = eventBody.querySelectorAll('.decision-choice-btn');
    btns.forEach(function(btn){
      btn.addEventListener('click', function(){
        var key = btn.getAttribute('data-key');
        ...
        addNews('🎥 '+evt.title+' — '+escapeHtml(choice.label)+' ('+escapeHtml(movie.title)+')');
        movie.eventLog.push({ phase:'Shoot — Week '+shoot.weekIndex, week:game.processedWeek, title:evt.title, choice:choice.label, outcome:outcome });
        renderHeader();
...
export function openPostProductionEvent(movie){
    ...
    renderDecisionCard('🎬 POST-PRODUCTION DECISION', evt, movie);
    var btns = eventBody.querySelectorAll('.decision-choice-btn');
    btns.forEach(function(btn){
      btn.addEventListener('click', function(){
        var key = btn.getAttribute('data-key');
        ...
        addNews('🎬 '+evt.title+' — '+escapeHtml(choice.label)+' ('+escapeHtml(movie.title)+')');
        movie.eventLog.push({ phase:'Post-Production', week:game.processedWeek, title:evt.title, choice:choice.label, outcome:outcome });
        renderHeader();
        renderDecisionOutcome(outcome, 'Continue to Test Screening →');
        $('eventContinueBtn').addEventListener('click', function(){
          eventModal.classList.add('hidden');
          renderObjectiveCard();
          computeFilmMetrics(movie);
...
export function renderTestScreeningModal(movie, feedback){
    ...
    testScreeningChoices.innerHTML = TEST_SCREENING_CHOICES.map(function(c){
      ...
      var targetsWeakest = c.targets && c.targets.indexOf(weakest.key)!==-1;
      var recTag = targetsWeakest ? '<span class="choice-tag choice-tag-rec">★ Targets weak spot</span>' : '';
      return '<button type="button" class="decision-choice-btn'+(targetsWeakest?' recommended':'')+'" data-choice="'+c.id+'">'+
        '<div class="choice-label">'+escapeHtml(c.label)+'</div>'+
        '<div class="choice-desc">'+escapeHtml(c.desc)+'</div>'+
        '<div class="choice-tags">'+recTag+'<span class="choice-tag">'+costLabel+'</span><span class="choice-tag">'+delayLabel+'</span></div>'+
        '</button>';
    }).join('');

    var btns = testScreeningChoices.querySelectorAll('.decision-choice-btn');
    btns.forEach(function(btn){
      btn.addEventListener('click', function(){
        ...
        movie.eventLog.push({ phase:'Test Screening', week:game.processedWeek, title:'Test Screening', choice:choiceDef.label, outcome:result.outcome });
        renderHeader();
...
export function showTestScreeningOutcome(outcome, onContinue){
    ...
    testScreeningSuggestion.innerHTML = '<div class="decision-outcome"><span class="outcome-flash">✓</span><p style="text-align:left;">'+outcome+'</p></div>'+
      '<button type="button" class="btn-primary" id="testScreeningContinueBtn" style="width:100%;margin-top:6px;">Continue to Release →</button>';
